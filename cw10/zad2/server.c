...
void start_local_server(){
    ...
    if((unix_socket = socket(AF_UNIX, SOCK_DGRAM, 0)) == -1){ print_error("unix_socket",errno);}
...
void start_network_server(){/*
    ...
    if((inet_socket = socket(AF_INET, SOCK_DGRAM,0)) == -1){ print_error("inet_scoket",errno);}
    ...
    while (1){
        ...
        for (int i = 0; i < MAX_NUM_OF_CLIENTS; i++) {
            if(clients[i] != NULL && clients[i]->opponent_id!=-1){
                ...
                if(sendto(clients[i]->fd, to_send, MAX_MESSAGE_LEN,0,clients[i]->addr,sizeof(struct sockaddr)) == -1){ print_error("send",errno);}
...
/*    shutdown(clients[client_id]->fd, SHUT_RDWR);
...
void disconnect_pair(int client_id){
    ...
    if(client_to_disconnect->has_opponent){
        if(sendto(client_to_disconnect->opponent_fd,mess,MAX_MESSAGE_LEN,0,clients[client_to_disconnect->opponent_id]->addr,sizeof(struct sockaddr)) == -1){print_error("send dis",errno);}
...
void no_ping_response(int i){
    ...
    char mess[MAX_MESSAGE_LEN];
    sprintf(mess,"%d##",DISCONNECT);
    printf("No ping res %s",clients[i]->login);
    if(sendto(client_to_disconnect->fd,mess,MAX_MESSAGE_LEN,0,client_to_disconnect->addr,sizeof(struct sockaddr)) == -1){print_error("send",errno);}
    disconnect_pair(i);
}
...
void save_client(int fd, char* name, struct sockaddr* addr){
    int new_id = get_id(name);
    char mess[MAX_MESSAGE_LEN];
    if(new_id != -1){
        printf("Name taken: %s\n", name);
        sprintf(mess,"%d##",NAME_TAKEN);
        if(sendto(fd,mess,MAX_MESSAGE_LEN,0,addr,sizeof(struct sockaddr)) == -1){ print_error("send",errno);}
        close(fd);
        return;
    }
    for (int i = 0; i < MAX_NUM_OF_CLIENTS; i++) {
    ...
    if(new_client->opponent_id != -1){
        ...
        if(sendto(new_client->opponent_fd, mess,MAX_MESSAGE_LEN,0, clients[new_client->opponent_id]->addr, sizeof(struct sockaddr)) == -1){ print_error("send",errno);}
        sprintf(mess,"%d# #", MOVE);
        if(sendto(new_client->opponent_fd, mess,MAX_MESSAGE_LEN,0, clients[new_client->opponent_id]->addr, sizeof(struct sockaddr)) == -1){ print_error("send",errno);}
    }
...
void make_mark(int id, char* args){
    ...
    if(sendto(clients[id]->opponent_fd,mess,MAX_MESSAGE_LEN,0, clients[clients[id]->opponent_id]->addr, sizeof(struct sockaddr)) == -1){ print_error("send",errno);}
}
...
void run_server(){
    while (1){
        ...
        if(recvfrom(fd_to_service,message,MAX_MESSAGE_LEN,0,addr,&len) == -1) { print_error("recv run",errno);}
        ...
        switch (msq_type) {
            case CONNECT:
                save_client(fd_to_service,name, addr);
                break;
            case PING:
